...
from prometheus_scrape.prometheus_scrape import PrometheusRulesProvider
...
class TestReloadAlertRules(unittest.TestCase):
    ...
    NO_ALERTS = json.dumps(
        {}
    )  # relation data representation for the case of "no alerts"

    # use a short-form free-standing alert, for brevity
    ALERT = yaml.safe_dump(
        {"alert": "free_standing", "expr": "avg(some_vector[5m]) > 5"}
    )
    ...
    def setUp(self):
        ...
        class ConsumerCharm(CharmBase):
            ...
            def __init__(self, *args, **kwargs):
                super().__init__(*args)
                self.rules_provider = PrometheusRulesProvider(
                    self, dir_path=alert_rules_path
                )
    ...
    def test_reload_after_dir_is_populated_updates_relation_data(self):
        ...
        self.sandbox.put_file(
            os.path.join(self.alert_rules_path, "alert.rule"), self.ALERT
        )
    ...
    def test_only_files_with_rule_or_rules_suffixes_are_loaded(self):
        ...
        for filename in filenames:
            ...
            rule_file = yaml.safe_dump(
                {"alert": filename, "expr": "avg(some_vector[5m]) > 5"}
            )

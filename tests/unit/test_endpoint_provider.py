...
class TestEndpointProvider(unittest.TestCase):
    ...
    def test_provider_default_scrape_relations_not_in_meta(self):
        """Tests that the Provider raises exception when no prometheus-scrape in meta."""
        harness = Harness(
            EndpointProviderCharm,
            # No provider relation with `prometheus-scrape` as interface
            meta="""
                name: provider-tester
                containers:
                    prometheus:
    ...
    def test_provider_default_scrape_relation_wrong_interface(self):
        ...
        harness = Harness(
            ...
            meta="""
                ...
                provides:
                    metrics-endpoint:
                        interface: not_prometheus-scrape
                """,
        )
...
class TestAlertRulesWithOneRulePerFile(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.topology = ProviderTopology(
            "MyModel", "MyUUID", "MyApp", "MyUnit", "MyCharm"
        )
    ...
    def test_alerts_in_both_formats_are_recursively_aggregated(self):
        ...
        self.assertEqual(
            {}, DeepDiff(expected_rules_file, rules_file_dict, ignore_order=True)
        )
            ...
            "groups": [
                {
                    ...
                    "rules": [
                        self.gen_rule(
                            "same", labels=self.topology.as_promql_label_dict()
                        ),
                        self.gen_rule(
                            "same", labels=self.topology.as_promql_label_dict()
                        ),
            ...
            "groups": [
                {
                    ...
                    "rules": [
                        self.gen_rule(0, labels=self.topology.as_promql_label_dict())
                    ],
                },
                {
...
class TestAlertRulesContainingUnitTopology(unittest.TestCase):
    ...
    def test_unit_label_is_retained_if_hard_coded(self):
        ...
        alert_rules = json.loads(
            relation.data[self.harness.charm.app].get("alert_rules")
        )

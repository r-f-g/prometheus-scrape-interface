...
from prometheus_scrape import ALLOWED_KEYS, MetricsEndpointConsumer
...
class EndpointConsumerCharm(CharmBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.framework.observe(
            self.prometheus_consumer.on.targets_changed, self.record_events
        )
...
class TestEndpointConsumer(unittest.TestCase):
    def setUp(self):
        metadata_file = open(
            "examples/operator/prometheus-scrape-consumer/metadata.yaml"
        )
    ...
    def test_consumer_accepts_rules_with_no_identifier(self):
        ...
        with self.assertLogs(level="DEBUG") as logger:
            ...
            messages = logger.output
            self.assertIn(
                "Alert rules were found but no usable labels were present", messages[1]
            )
            self.assertIn(
...
def job_name_suffix(job_name, labels, rel_id):
    ...
    return job_name[len(name_prefix) :]  # noqa: E203

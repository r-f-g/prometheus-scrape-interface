...
from prometheus_scrape.prometheus_scrape import MetricsEndpointAggregator
...
class TestEndpointAggregator(unittest.TestCase):
    ...
    def test_adding_prometheus_then_target_forwards_a_labeled_alert_rule(self):
        ...
        alert_rules_rel_id = self.harness.add_relation(
            ALERT_RULES_RELATION, "rules-app"
        )
        ...
        expected_group = {
            ...
            "rules": [
                {
                    "alert": "CPU_Usage",
                    "expr": 'cpu_usage_idle{is_container!="True", '
                    'group="promoagents-juju"} < 10',
                    "for": "5m",
                    "labels": {
                        "override_group_by": "host",
                        "severity": "page",
                    ...
                    "annotations": {
                        "description": "Host {{ $labels.host }} has had < 10% idle cpu "
                        "for the last 5m\n",
                        "summary": "Host {{ $labels.host }} CPU free is less than 10%",
                    },
                }
    ...
    def test_scrape_jobs_from_multiple_target_applications_are_forwarded(self):
        ...
        target_rel_id_1 = self.harness.add_relation(
            SCRAPE_TARGET_RELATION, "target-app-1"
        )
        ...
        target_rel_id_2 = self.harness.add_relation(
            SCRAPE_TARGET_RELATION, "target-app-2"
        )
        ...
        expected_groups = [
                ...
                "rules": [
                    {
                        ...
                        "annotations": {
                            "description": "Host {{ $labels.host }} has had < 10% idle "
                            "cpu for the last 5m\n",
                            "summary": "Host {{ $labels.host }} CPU free is less than 10%",
                        },
                    }
            ...
            },
            {
                ...
                "rules": [
                    {
                        "alert": "DiskFull",
                        "expr": 'disk_free{is_container!="True", '
                        'fstype!~".*tmpfs|squashfs|overlay"}  <1024',
                        "for": "5m",
                        "labels": {
                            "override_group_by": "host",
                            "severity": "page",
                        ...
                        "annotations": {
                            "description": "Host {{ $labels.host}} {{ $labels.path }} "
                ...
                "rules": [
                    {
                        ...
                        "annotations": {
                            "description": "Host {{ $labels.host }} has had < 10% idle "
                            "cpu for the last 5m\n",
                            "summary": "Host {{ $labels.host }} CPU free is less than "
                            "10%",
                        },
                    }
                ],
